...
class Wechat {
  ...
  async createMenu (menu) {
    try {
      ...
      const {access_token} = await this.fetchAccessToken();
      //定义请求地址
      const url = `${prefix}menu/create?access_token=${access_token}`;
      //发送请求
  ...
  async deleteMenu () {
    try {
      ...
      const {access_token} = await this.fetchAccessToken();
      //定义请求地址
      const url = `${prefix}menu/delete?access_token=${access_token}`;
      //发送请求
      const result = await rp({method: 'GET', url, json: true});
  ...
  async createTag(name){
    try{
      ...
      //发送请求
      const result = await rp({method: 'GET', url, json: true,body:{tag: {name}}});
      return result;
  ...
  //删除
  async batchUsersTag(tagid,next_openid){
    try{
